...
export function getTabularDownloadData(geneticTracks, clinicalTracks, heatmapTracks, genericAssayHeatmapTracks, genesetTracks, uidOrder, getCaseId, columnMode, distinguishDrivers) {
  function getCountsTrackRowLabel(attrName, countsCategory) {
  ...
  for (const geneticTrack of geneticTracks) {
    ...
    for (const geneticTrackDatum of currentTrackData) {
      ...
      if (geneticTrackDatum.disp_structuralVariant !== undefined) {
        oncoprintData.STRUCTURAL_VARIANT[currentGeneName][id] = structuralVariantMap[geneticTrackDatum.disp_structuralVariant + '']
          ? structuralVariantMap[geneticTrackDatum.disp_structuralVariant + '']
      ...
      if (geneticTrackDatum.disp_mut !== undefined) {
        oncoprintData.MUTATIONS[currentGeneName][id] = mutationMap[geneticTrackDatum.disp_mut]
  ...
  for (const clinicalTrack of clinicalTracks) {
    ...
    if (clinicalTrack.datatype === 'counts') {
      for (const category of clinicalTrack.countsCategoryLabels) {
        const rowLabel = getCountsTrackRowLabel(currentAttributeName, category);
    ...
    for (const clinicalTrackDatum of currentClinicalTrackData) {
      ...
      if (clinicalTrack.datatype === 'counts') {
        for (const category of clinicalTrack.countsCategoryLabels) {
          oncoprintData.CLINICAL[getCountsTrackRowLabel(currentAttributeName, category)][id] = clinicalTrackDatum.attr_val
            ? clinicalTrackDatum.attr_val[category]
            : '';
        }
      }
      else {
        oncoprintData.CLINICAL[currentAttributeName][id] = '';
        oncoprintData.CLINICAL[currentAttributeName][id] = ifNotDefined(clinicalTrackDatum.attr_val, '');
      }
    }
  }
  //Add heatmap data
  const exportedHeatmapTracks = heatmapTracks
    .concat(genericAssayHeatmapTracks)
    .concat(genesetTracks);
  for (const heatmapTrack of exportedHeatmapTracks) {
    const currentHeatmapGene = heatmapTrack.label;
    const currentHeatmapType = 'HEATMAP ' +
    ...
    for (const heatmapTrackDatum of currentHeatmapTrackData) {
      ...
      if (oncoprintData[currentHeatmapType][currentHeatmapGene] ===
        undefined) {
  ...
  Object.keys(oncoprintData).forEach(function (dataType) {
    Object.keys(oncoprintData[dataType]).forEach(function (geneOrClinicalAttribute) {
      content += geneOrClinicalAttribute + '\t' + dataType;
      for (let l = 0; l < caseNames.length; l++) {
        content +=
          '\t' +
          oncoprintData[dataType][geneOrClinicalAttribute][caseNames[l]];
      }
      content += '\n';
    });
  });
...
export default function tabularDownload(geneticTracks, clinicalTracks, heatmapTracks, genericAssayHeatmapTracks, genesetTracks, uidOrder, getCaseId, columnMode, distinguishDrivers) {
  const prefixName = columnMode === 'sample' ? 'SAMPLE_DATA_' : 'PATIENT_DATA_'; //Name depending on the type of case
  fileDownload(getTabularDownloadData(geneticTracks, clinicalTracks, heatmapTracks, genericAssayHeatmapTracks, genesetTracks, uidOrder, getCaseId, columnMode, distinguishDrivers), prefixName + 'oncoprint.tsv');
}
