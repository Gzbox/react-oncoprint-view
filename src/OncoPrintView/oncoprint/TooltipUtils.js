...
function sampleViewAnchorTag(study_id, sample_id) {
  return `<a class="nobreak" href="${getSampleViewUrl(study_id, sample_id)}" target="_blank">${sample_id}</a>`;
}
function patientViewAnchorTag(study_id, patient_id) {
  return `<a class="nobreak" href="${getPatientViewUrl(study_id, patient_id)}" target="_blank">${patient_id}</a>`;
}
export function makeGenePanelPopupLink(gene_panel_id, profiled, numSamples) {
  let anchor = $(`<span class="nobreak"><a href="#" ${!profiled ? 'style="color:red;"' : ''} oncontextmenu="return false;">${gene_panel_id}</a>${numSamples ? ` (${numSamples})` : ''}</span>`);
  anchor.ready(() => {
...
export function makeCategoricalTrackTooltip(track, link_id) {
  return function (dataUnderMouse) {
    ...
    if (naCount > 0) {
      ret += `${track.label}: <b>N/A</b>${dataUnderMouse.length > 1 ? ` (${naCount} samples)` : ''}<br/>`;
    }
    return $('<div>')
  ...
  return function (dataUnderMouse) {
    ...
    if (trackSpec.tooltipValueLabel) {
      ...
      switch (trackSpec.molecularAlterationType) {
        ...
        case AlterationTypeConstants.GENERIC_ASSAY:
          ...
          data_header = `${deriveDisplayTextFromGenericAssayType(trackSpec.genericAssayType)}: `;
          break;
        default:
          data_header = 'Value: ';
          break;
    ...
    for (const d of dataUnderMouse) {
      if (d.profile_data !== null &&
        ...
        if (trackSpec.molecularAlterationType ===
...
export function makeGeneticTrackTooltip_getCoverageInformation(profiled_in, not_profiled_in, alterationTypesInQuery, molecularProfileIdToMolecularProfile) {
  ...
  if (profiled_in) {
    dispProfiledGenePanelIds = _.uniq(profiled_in.map(x => x.genePanelId).filter(x => !!x));
    dispProfiledIn = _.uniq(profiled_in.map(x => x.molecularProfileId));
    if (molecularProfileIdToMolecularProfile) {
      profiledInTypes = _.keyBy(dispProfiledIn, molecularProfileId => molecularProfileIdToMolecularProfile[molecularProfileId]
  ...
  if (not_profiled_in) {
    ...
    if (profiledInTypes &&
      ...
      let notProfiledInTypes = _.keyBy(dispNotProfiledIn, molecularProfileId => molecularProfileIdToMolecularProfile[molecularProfileId]
        .molecularAlterationType);
      ...
      dispNotProfiledIn = dispNotProfiledIn.concat(alterationTypesInQuery
        .filter(t => !profiledInTypes[t] && !notProfiledInTypes[t])
        .map(t => AlterationTypeText[t]));
    }
    dispNotProfiledGenePanelIds = _.uniq(not_profiled_in.map(x => x.genePanelId)).filter(x => !!x && !dispProfiledGenePanelIdsMap[x]);
  }
  const dispAllProfiled = !!(dispProfiledIn &&
    dispProfiledIn.length &&
    dispNotProfiledIn &&
    !dispNotProfiledIn.length);
  const dispNotProfiled = !!(dispNotProfiledIn &&
    dispNotProfiledIn.length &&
    dispProfiledIn &&
    !dispProfiledIn.length);
  return {
  ...
  if (dataUnderMouse[0].sample) {
    if (dataUnderMouse.length > 1) {
      caseIdElt = caseViewLinkout
        ? `<a class="nobreak" href=${getSampleViewUrl(dataUnderMouse[0].study_id, dataUnderMouse[0].sample, dataUnderMouse.map(d => ({
          studyId: d.study_id,
          patientId: d.patient,
      ...
      caseIdElt = caseViewLinkout
        ? sampleViewAnchorTag(dataUnderMouse[0].study_id, dataUnderMouse[0].sample)
    ...
    if (dataUnderMouse.length > 1) {
      caseIdElt = caseViewLinkout
        ? `<a class="nobreak" href=${getPatientViewUrl(dataUnderMouse[0].study_id, dataUnderMouse[0].patient, dataUnderMouse.map(d => ({
          studyId: d.study_id,
          patientId: d.patient,
      ...
      caseIdElt = caseViewLinkout
        ? patientViewAnchorTag(dataUnderMouse[0].study_id, dataUnderMouse[0].patient)
